...
with open("Output.vhd", "w") as vhdFile:
	...
	print("architecture Behavioral of TwosCompSquarer is\n", file=vhdFile)
	...
	print("END Behavioral;", file=vhdFile)
vhdFile.close()

with open("Output2.vhd", "w") as vhdFile2:
	print("--Square", file=vhdFile2)
	print("library ieee;", file=vhdFile2)
	print("use ieee.std_logic_1164.all;\n", file=vhdFile2)
	print("entity SquareSquarer is", file=vhdFile2)
	...
	print("		);", file=vhdFile2)
	print("end SquareSquarer;\n", file=vhdFile2)
	print("architecture Behavioral of SquareSquarer is\n", file=vhdFile2)
	print("begin", file=vhdFile2)
	for j in iteration(3*nBits, 6*nBits-1, 1): # this is the MSB position
		count = 0
		for m in iteration(0, np.power(2,nBits)/2, 1): # this is the row	
			if(truthTable2[m,j] == 1):
				count = count + 1 # this counter keeps track of how many times a 1 shows up for a given output bit
		if(count > 0):
			print("y" + "(" + str(3*nBits*2-1-j) + ")" + " <= ", file=vhdFile2)
		count2 = 0 # this counter keeps track of how many times a 1 bit has been accounted for
		for k in iteration(0, np.power(2,nBits)/2, 1): # this is the row	
	...
	print("END Behavioral;", file=vhdFile2)
vhdFile2.close()
